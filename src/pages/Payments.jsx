...
export default function Payments() {
  ...
  const load = () => api.get("/payments/monitor").then((r) => setRows(Array.isArray(r.data) ? r.data : []));
  useEffect(() => { load(); }, []);
      ...
      <div className="p-4 lg:p-8">
        ...
        <div className="bg-white rounded-lg border border-slate-200 overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filtered.map((r, idx) => (
                <TableRow key={r.ecp_id ? `ecp-${r.ecp_id}` : `row-${idx}`} data-testid={`pay-row-${r.ecp_id || idx}`} className="hover:bg-slate-50">
                  <TableCell>
        ...
        <DialogContent className="max-w-2xl">
          ...
          {detail && (
            <Table>
              ...
              <TableBody>
                {(!detail.payments || detail.payments.length === 0) && <TableRow><TableCell colSpan={5} className="text-slate-400 py-4 text-sm">No payments yet.</TableCell></TableRow>}
                {(detail.payments || []).map((p, pIdx) => (
                  <TableRow key={p.id ? `pay-${p.id}` : `p-${pIdx}`}>
                    ...
                    <TableCell className="text-xs text-slate-500">{p.updated_by_name || p.recorded_by_name || "—"}</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
